...
class NamingSeries:
    ...
    def get_preview(self, doc=None) -> list[str]:
        ...
        for count in range(1, 4):
            ...
            generated_names.append(parse_naming_series(
                self.series, doc=doc, number_generator=fake_counter))
    ...
    def update_counter(self, new_count: int) -> None:
        ...
        if frappe.db.get_value("Series", prefix, "name", order_by="name") is None:
            frappe.qb.into(Series).insert(
                prefix, 0).columns("name", "current").run()

        (
            frappe.qb.update(Series).set(Series.current, cint(
                new_count)).where(Series.name == prefix)
        ).run()
    ...
    for e in parts:
        ...
        if e.startswith("#"):
            if not series_set:
                ...
                if hash_:
                    print(doctype)
                    part = frappe.generate_hash(part, digits).upper()
                series_set = True
        elif e == "YY":
...
def getseries(key, digits):
    ...
    current = (frappe.qb.from_(series).where(
        series.name == key).for_update().select("current")).run()
...
def revert_series_if_last(key, name, doc=None):
    ...
    *Example:*
    ...
    series = DocType("Series")
    current = (
        frappe.qb.from_(series).where(
            series.name == prefix).for_update().select("current")
    ).run()
...
def get_default_naming_series(doctype: str) -> str | None:
    ...
    naming_series_options = frappe.get_meta(
        doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str):
    ...
    if not frappe.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
        frappe.throw(_("Name of {0} cannot be {1}").format(
            doctype, name), frappe.NameError)

    special_characters = "<>"
    if re.findall(f"[{special_characters}]+", name):
        message = ", ".join(f"'{c}'" for c in special_characters)
        frappe.throw(
            _("Name cannot contain special characters like {0}").format(
                message), frappe.NameError
        )
...
def _set_amended_name(doc):
    ...
    if frappe.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
        ...
        am_prefix = "-".join(doc.amended_from.split("-")
                             [:-1])  # except the last hyphen
